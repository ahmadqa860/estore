...
<div class="row">
    <div class="col-md-6">
        <form action="{{ url('cms/content/'.$item['id']) }}" method="POST" novalidate autocomplete="off">
            @csrf
            {{ method_field('PUT') }}
            <div class="form-group">
                ...
                <select name="menu_id" id="menu-id" class="form-control">
                    @foreach($menu as $menu_item)
                        <option @if( $item['menu_id'] == $menu_item['id'] ) selected @endif  value="{{ $menu_item['id'] }}">{{ $menu_item['link'] }}</option>
                    @endforeach
                </select>
